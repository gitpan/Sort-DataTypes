...
The list can be a list of pure IPs or the CIDR representation including
the netmask (A.B.C.D/MASK).

When sorting CIDR representations, if the IP part of two elements is
identical, the following two rules are used:
