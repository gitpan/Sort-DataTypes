...
The elements of @list are sorted. Each element in @list must be a key
in %hash, and the value of that key must be of the appropriate
type. The elements of @list are sorted by using the cmp_METHOD
function to compare the values in the hash.
...
These sort domain names (foo.bar.com) or anything else consisting of a class,
subclass, subsubclass, etc., with the most significant class at the right (i.e.
subsubclass.subclass.class).

Each element in the list is split into subvalues. Subvalues in a domain are
separated from each other by a period (.) by default, but this can be
overridden.  If $sep is passed in, it is a regular expression to split the
values into subvalues.
